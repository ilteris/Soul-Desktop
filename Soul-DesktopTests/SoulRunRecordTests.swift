...
struct SoulRunRecordTests {
    ...
    @Test func orchestrationStatusResultDecodesDaemonSnapshot() throws {
        ...
        #expect(result.snapshot.projectBinding?.statusLabel == "tilde_home · portable")
        #expect(result.snapshot.projectBinding?.locationSummary == "~/Code/Soul-Desktop → /Users/adele/Code/Soul-Desktop")
        #expect(result.snapshot.projectBinding?.manifestSummary == "~/soul-cli/soul/config/PROJECTS.json")
        #expect(result.snapshot.projectBinding?.healthSummary == "Path exists")
